...
DATABASE_URL = os.getenv('DATABASE_URL', 'sqlite:///' + os.path.join(BASE_DIR, 'data.db'))
...
class User(db.Model):
    ...
    role = db.Column(db.String(30), default='user')  # admin / user / guest
...
@app.route('/')
def index():
    # If no admin: redirect to setup page
...
def setup_admin():
    # First-run admin creation. Once an admin exists this page redirects to index.
    ...
    if request.method == 'POST':
        ...
        if not username or not password:
            ...
            return redirect(url_for('setup_admin'))
        admin = User(email=email, username=username, role='admin')
        admin.set_password(password)
        db.session.add(admin); db.session.commit()
        flash('تم إنشاء حساب المدير. يمكنك الآن تسجيل الدخول.', 'success')
...
def register():
    if request.method == 'POST':
        ...
        # For guests: allow name only (no password) and role guest
        if not username:
            flash('اكتب اسم المستخدم', 'warning')
            return redirect(url_for('register'))
        if User.query.filter_by(username=username).first():
            flash('الاسم مستخدم بالفعل', 'warning')
            return redirect(url_for('register'))
        # If password provided -> normal user, else guest
        if password:
...
def login():
    if request.method == 'POST':
        ...
        if not user:
            ...
            return redirect(url_for('login'))
        # if guest with no password allow login by username only
...
def handle_join(data):
    ...
    if not room:
        return
    join_room(room_slug)
    msg = {'user':'system','text': f'{username} دخل الغرفة', 'ts': datetime.utcnow().isoformat()}
    emit('system', msg, to=room_slug)
...
def handle_leave(data):
    ...
    emit('system', {'user':'system','text': f'{username} خرج من الغرفة', 'ts': datetime.utcnow().isoformat()}, to=room_slug)
