...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS products (
        id INTEGER PRIMARY KEY, title TEXT, desc TEXT, price REAL, qty INTEGER, image TEXT
    )""")
...
@app.route("/")
def index():
    prods = query_db("SELECT id,title,desc,price,qty,image FROM products")
    return render_template("index.html", products=prods)

@app.route("/product/<int:pid>")
def product(pid):
    p = query_db("SELECT id,title,desc,price,qty,image FROM products WHERE id=?", (pid,), one=True)
    if not p:
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            flash("المستخدم أو الإيميل مستخدم") 
...
def checkout():
    ...
    if request.method == "POST":
        ...
        conn.close()
        session["cart"] = {}
        flash("تم إنشاء الطلب — (دفع تجريبي)")
        return redirect(url_for("index"))
...
def admin():
    if request.method == "POST":
        pwd = request.form.get("adminpass")
        if pwd != ADMIN_PASS:
        ...
        if "add_product" in request.form:
            ...
            query_db("INSERT INTO products (title,desc,price,qty,image) VALUES (?,?,?,?,?)",
                     (title,desc,price,qty,imgname))
