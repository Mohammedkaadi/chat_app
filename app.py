...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO rooms (id,name,description) VALUES (1,'general','General room')")
    conn.commit()
    conn.close()
...
def signup():
    ...
    try:
        ...
        return 'Username or email already used', 400
...
def forgot():
    if request.method == 'POST':
        ...
        if not user:
            return render_template('forgot.html', message='No account for this email')
        ...
        # SMTP optional: in demo print to console
        smtp_user = os.environ.get('SMTP_USER')
        if smtp_user:
            # Implement sending using SMTP provider if configured via env vars
            pass
        else:
            print('[DEBUG] Password reset link for', email, reset_link)
        return render_template('forgot.html', message='Reset link sent (check console in demo).')
...
# file upload in chat (optional endpoint for direct uploads)
